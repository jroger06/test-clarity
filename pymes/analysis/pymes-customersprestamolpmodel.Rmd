...
---
---
title: 'Pymes Analytics - Modelo de Préstamos LP Clientes'
...
title     <- '[Pymes]: Modelo de Préstamos LP Clientes'
keywords  <- 'customer,prestamo,pymes'  
```
...
summarizeModelResults <- function (name, model, train, sample, test) {  
  ...
  if(nrow(model$results)>1){plot(model, main = "Model performance")}
...
Modelos analíticos de propensión a contratación o tenencia de Préstamos LP.

### 2. Objetivo

Detección de Pymes propensas a nueva contratación o tenencia en otras entidades bancarias de Préstamos LP.
...
* **da_pymes.tablon_clientes_prestamos:** Indicador de si el cliente tiene préstamo (CP/LP) actualmente.
...
Obtenemos para cada cliente sus datos (del tablón) y la variable de si tiene préstamo LP actualmente.

```{r getData, cache=TRUE, echo=FALSE, eval = TRUE}

# q.dataset_prestamolp <- "
# SELECT cli.*, p.*, cli2.cod_cnae_1
...
# LEFT JOIN da_pymes.tablon_clientes_prestamos p ON cli.cod_persctpn = p.cod_persona
# "

q.dataset_prestamolp <- "
SELECT cli.*, p.*, pbal.prestamolp_balances_current_imp, cli2.cod_cnae_1
FROM da_pymes.tablon_clientes_sinfo cli
join da_pymes.tablon_clientes cli2 on cli.cod_persctpn = cast(trim(cli2.cod_persctpn) as int)
LEFT JOIN da_pymes.tablon_clientes_prestamos p ON cli.cod_persctpn = p.cod_persona
LEFT JOIN
(
select cod_persctpn as cod_persona,
NVL(cuenta_31220,0) as prestamolp_balances_current_imp
from da_pymes.tablon_balances_cabeceras bal
join (select cod_documps1, cod_persctpn, cod_entalfa from da_pro.clientes_corp where partition_id='20160131') cli
on trim(cli.cod_documps1) = trim(bal.nif) and cli.cod_entalfa = '0182'
where cuenta_31220>0
)  pbal
ON cli.cod_persctpn = cast(trim(pbal.cod_persona) as int)
"

df.all <- qimpala(q.dataset_prestamolp)
df.all <- df.all[,-4]
...
Los clientes que observamos que tienen préstamo LP los marcamos como certeza. Utilizamos para ello la tenencia interna en el banco (a partir de datos de SINFO), balances (cuenta_31220) y cirbe (imp_fina_ban, imp_fina_sba).
...
dd.output$prob_prestamolp <- 0
dd.output[df.all$ind_prest_largo > 0 |
            df.all$prestamolp_balances_current_imp > 0 |
            df.all$imp_fina_ban > 0 |
            df.all$imp_fina_sba > 0,]$prob_prestamolp = 100

# dd.output[df.all$total_prestamolp_two_years_n > 0 | df.all$prestamolp_balances_two_years_imp > 0, ]$prob_prestamolp = 95

```

Así queda la distribución de la varibles tras esa asignación:
```{r, echo=FALSE}
table(dd.output$prob_prestamolp)
```
...
# nuevas variables
# df.all$imp_activo_new <- df.all$imp_activo - df.all$imp_prestamolp
df.all$imp_rid_sba_new <- df.all$imp_rid_sba - df.all$imp_fina_sba
df.all$imp_rid_ban_new <- df.all$imp_rid_ban - df.all$imp_fina_ban
...
Ahora asignamos la clase para entrenar. Consideramos TRUE si tiene un préstamo LP, FALSE en otro caso. 

```{r chunk2, cache=FALSE, echo=FALSE, eval = TRUE}
df.all$Class <- (dd.output$prob_prestamolp > 0)
...
Como el ratio de positivos está sobre el 20%, no hacemos subsampling sobre los negativos en principio. Repartimos en train (80%) y test (20%). Ajustamos cada modelo y al final se comparar la especificity, sensitivity y AUC.
...
train.prestamolp <- df[index,]         
test.prestamolp  <- df[-index,]         
table(train.prestamolp$Class)
table(test.prestamolp$Class)

## if negative, use all instances for training 
if (n.sample.train < 0) { n.sample.train = nrow(train.prestamolp) }
if (n.sample.train > nrow(train.prestamolp) ) {n.sample.train = nrow(train.prestamolp)}

sample.train <- sample(seq(1,nrow(train.prestamolp)), n.sample.train )
...
#             
            ...
            # variables de clase
             "Class", "ind_prest_largo",
            
            # correlan mucho o las queremos quitar
             "prestamolp_balances_current_imp",
             "ind_cir",
             "imp_activo", "imp_activo_evol",
             "imp_rid_sba", "imp_rid_ban", "per_cir_rid",
             "imp_rin_sba", "imp_rin_ban", "per_cir_rin",
             "imp_rgar_sba", "imp_rgar_ban", "per_cir_rgar", 
             "imp_gar_sba","imp_gar_ban", "per_cir_gar",
             "imp_fina_ban", "imp_fina_sba", "per_cir_fina",
             "imp_margen"
   )
...
```{r PrestamoCP, eval=FALSE}

# ¿Debemos excluir la variable de tenencia de préstamo CP?

A = data.table(data.frame(prestamolp_sba=df.all$imp_fina_sba,
                          prestamolp_ban=df.all$imp_fina_ban,
                          prestamocp_sba=df.all$imp_fin_sba,
                          prestamocp_ban=df.all$imp_fin_ban))

A$i_prestamolp_sba = ifelse(A$prestamolp_sba>0,1,0)
A$i_prestamolp_ban = ifelse(A$prestamolp_ban>0,1,0)
A$i_prestamocp_sba = ifelse(A$prestamocp_sba>0,1,0)
A$i_prestamocp_ban = ifelse(A$prestamocp_ban>0,1,0)

sum(A$i_prestamolp_sba)
sum(A$i_prestamolp_ban)
sum(A$i_prestamocp_sba)
sum(A$i_prestamocp_ban)

A[,list(corto_ban=sum(i_prestamocp_ban), corto_sba=sum(i_prestamocp_sba), largo_sba=sum(i_prestamolp_sba)),by="i_prestamolp_ban"]
A[,list(largo_ban=sum(i_prestamolp_ban), largo_sba=sum(i_prestamolp_sba), corto_sba=sum(i_prestamocp_sba)),by="i_prestamocp_ban"]

# Aproximadamente el 50% de los clientes que tienen préstamo LP tienen a su vez préstamo CP, y viceversa. La dejamos en el modelo.
...
```{r train GBM, message=FALSE, fig.width=16, fig.height=16, cache=TRUE}

# set.seed(42)
# model.gbm <- train(model.formula, data = train.prestamolp, 
#                     method = "gbm",
#                     subset = sample.train,
#                     trControl = ctrl,
#                     verbose = FALSE,
...
model.gbm <- readRDS(paste(model_dirpath, 
                    "customers.prestamolp.gbm.Rda",
                    sep = ""))

#model.gbm
varImp(model.gbm)
summarizeModelResults("Propensity Préstamo LP - GBM", model.gbm,  train.prestamolp, sample.train, test.prestamolp)

```


## Train a Random Forest
```{r train random forest, message=FALSE, fig.width=16, fig.height=16, cache=TRUE}

set.seed(42)
# system.time(
#   model.rf <- train(model.formula, data = train.prestamolp, 
#                   method = "rf",
#                   subset = sample.train,
#                   trControl = ctrl,
#                   verbose = FALSE,
...
model.rf <- readRDS(paste(model_dirpath, 
                    "customers.prestamolp.rf.Rda",
                    sep = ""))

varImp(model.rf)
summarizeModelResults("Propension Préstamo LP - Random Forest", model.rf,  train.prestamolp, sample.train, test.prestamolp)
...
set.seed(42)
# system.time(
#   model.glm <- train(model.formula, data = train.prestamolp, preProcess = c("center", "scale"),
...
model.glm <- readRDS(paste(model_dirpath, 
                    "customers.prestamolp.glm.Rda",
                    sep = ""))

#model.glm$finalModel

varImp(model.glm)
summarizeModelResults("Propension Préstamo LP - GLM", model.glm,  train.prestamolp, sample.train, test.prestamolp)

```

```{r save Models, eval=FALSE}

saveModel(model.gbm, "customers.prestamolp.gbm") # model_dirpath = model_dirpath si queremos cambiar el directorio de guardado del modelo

saveModel(model.rf, "customers.prestamolp.rf")

saveModel(model.glm, "customers.prestamolp.glm")
...
ggplot(subset(importance, modelo%in%c("gbm","rf")), aes(x=reorder(var, imp, max), y=imp, fill=modelo)) + geom_bar(stat="identity", position="dodge") +
  xlab("Variable") + ylab("Relative Importance") + coord_flip() + theme(axis.text.y = element_text(size=14))

ggplot(subset(importance, modelo%in%c("glm")), aes(x=reorder(var, imp, max), y=imp, fill=modelo)) + geom_bar(stat="identity", position="dodge") +
  xlab("Variable") + ylab("Relative Importance") + coord_flip() + theme(axis.text.y = element_text(size=14))+ scale_fill_manual(values=c("#66CC99"))

# varImp(model.glm)
# plot(log(coefficients(model.glm$finalModel)))

```

Estimamos la probabildiad para los datos de test. 
```{r}
results <- data.frame(cod_documps1=test.prestamolp$cod_idefisca,Class = test.prestamolp$Class)
results$GBM <- predict(model.gbm, 
                       newdata = test.prestamolp, 
                       type = "prob")[, 'Yes']
...
ggplot(results, aes(x=GBM, color=Class)) + geom_density() + xlab("Estimated Probability") + ggtitle("Gradient Boosting Machine")
ggplot(results, aes(x=RF, color=Class)) + geom_density() + xlab("Estimated Probability") + ggtitle("Random Forest")
ggplot(results, aes(x=GLM, color=Class)) + geom_density() + xlab("Estimated Probability") + ggtitle("GLM")
```

# Some statistics 
Depending on at which point we consider as positive, we find some customers who doesn't have Préstamo LP currently but with high probability. We compute the estimated probability for every customer and precision and recall for some posible cuts.
...
ggplot(results.all, aes(x=GLM, color=Class)) + geom_density() + xlab("Estimated Probability") + ggtitle("GLM")
...
```{r probByCnae, fig.width=15, fig.height=15}
# por cnae
df.cnae <- results.all
...
Modelos con resultados aceptables. GBM seleccionado como modelo definitivo. Variables de importancia en los modelos significativas tienen sentido de negocio.
